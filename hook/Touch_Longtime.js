export default class Touch {
	...
	getOrigin(e) {
		...
		const org = {
			...
			distance: Math.sqrt(Math.pow(e.touches[0].clientX - e.touches[1].clientX, 2) + Math.pow(e.touches[0]
				.clientY - e.touches[1].clientY, 2))
		}
		return org;
	}
	touchStart(e) {
	...
	AngleMount() {
		var _X = this.end.x - this.start.x,
			_Y = this.end.y - this.start.y;
		//返回角度 Math.atan()返回数字的反正切值
		const C=360 * Math.atan(_Y / _X) / (2 * Math.PI);
	...
	Scale() {
		...
		// 		 this.touchends[index].x=e.clientX
		// 		 this.touchends[index].y=e.clientY
		// })
	}
